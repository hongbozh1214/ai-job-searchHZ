...
- **Personal data boundaries**: populated profiles, tracker, salary data, and application archives are gitignored to reduce accidental Git publication. They are kept in each agent's local checkout, but the configured model provider can receive candidate facts and excerpts when the agent reads them or you paste them in chat. Search providers and optional integrations also receive the queries or fields their workflows send; `/notion-sync` publishes pipeline fields and document filenames, not document contents. Check your runtime's provider, integrations, and data retention settings before adding sensitive material. Git ignore rules do not erase previously committed data or make a cloud model local.
