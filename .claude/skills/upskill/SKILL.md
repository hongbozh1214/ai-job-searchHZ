...
3. Read `job_scraper/seen_jobs.json`. Keep entries with `"status": "ranked"` and `rank_score >= 45` **only when `rank_eligible` is not `false` and none of `location_verdict`, `language_gate` or the recorded `market_gates` is `FAIL`** — the Moderate Fit floor from `04-job-evaluation.md` (below that, a job is Weak/Poor Fit and would otherwise dominate the heatmap with jobs the user shouldn't chase). Apply the veto to older rows without `rank_eligible` as well; do not recommend learning skills solely to pursue an excluded job. For each kept entry, note its `title`, `company`, `rank_score`, and — when present — its recorded `gaps`. An entry with no `gaps` field (ranked before gap persistence existed) is skipped, counted, and reported once in the terminal: *"N ranked jobs were scored before gap persistence and contribute nothing; `/rank --all` re-scores them."* Never back-fill a missing `gaps` field by guessing from the title.
4. Read `documents/profile/01-candidate-profile.md` to get the candidate's current skills and experience.
...
3. Read `documents/profile/01-candidate-profile.md` for the candidate's current skills.
...
1. **Dedupe.** Match tracker `source` URL to ranked `url` first; fall back to case-insensitive company + role (casefold + strip on both fields) only if a URL is absent and the pair is unambiguous — the same match `/notion-sync`'s Step 2 describes. Two postings at the same employer with identical role names but different known URLs contribute separately. A job present in both counts once.
...
Remove any skill from the list that is already present in the local candidate profile (`documents/profile/01-candidate-profile.md`). Be generous — if the profile mentions a skill in any form (e.g. "Python" covers "Python scripting"), remove it.
