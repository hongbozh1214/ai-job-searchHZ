...
**Every claim in a form field must already be defensible from the same local sources the CV and cover letter are grounded against** — the union of `documents/profile/01-candidate-profile.md`, the local master CV (`documents/profile/cv/main_example.tex`), and `documents/profile/CLAUDE.md`, with a claim grounded if ANY of the three supports it. The interviewer reads the form alongside the CV. A form field is not a place to introduce new claims, inflate scope, or fill space — it is a place to *select* from what is already true and arrange it for the question asked.
...
- [ ] Every factual claim traces to the union of `documents/profile/01-candidate-profile.md`, the local master CV (`documents/profile/cv/main_example.tex`), and `documents/profile/CLAUDE.md`
