...
3. Build the set of **open applications**: tracker rows whose `status` is not **Final** (per the **Tracker status vocabulary** in `/outcome`). For each, select `documents/applications/<slug>/` from its `posting_key:<slug>` or `china_posting_key:<slug>` marker in notes, or for legacy rows derive the slug by the **Subfolder naming** rule in `documents/README.md`. If multiple rows share the company and role, verify the source URL against the archive before using a legacy path; ask and skip ambiguous archives. Check whether `outcome.md` exists there. Reuse this exact derived path for any write in Step 7a.
