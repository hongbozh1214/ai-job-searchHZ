...
2. **`documents/applications/*/outcome.md`** — for each resolved application, read the outcome file to get the exact interview stages reached (the checkboxes) and any notes. Join using the tracker row’s `posting_key:<slug>` or `china_posting_key:<slug>` marker and the archive folder name. For legacy rows without a marker, match company+role (lowercase, ignore punctuation) only when unique and check source URL against saved posting provenance when available. Leave ambiguous archives unmatched and flag them in the report; never attach an unmatched archive to an arbitrary row.
...
- **Footer:** "Generated by ai-job-search · {ISO date}"
