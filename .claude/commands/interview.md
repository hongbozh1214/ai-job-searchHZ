...
1. **The archive** (started by `/apply`, maintained by `/outcome`): derive `<company>_<role>` by the **Subfolder naming** rule in `documents/README.md`, then use `documents/applications/<company>_<role>/`. For a China row with `china_posting_key:<slug>` in tracker notes, use `documents/applications/<slug>/` so two similarly named postings stay separate.
