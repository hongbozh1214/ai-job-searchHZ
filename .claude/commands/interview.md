...
1. **The archive** (started by `/apply`, maintained by `/outcome`): use this tracker row’s `posting_key:<slug>` or `china_posting_key:<slug>` marker in notes for `documents/applications/<slug>/`. For legacy rows derive `<company>_<role>` by the **Subfolder naming** rule in `documents/README.md` and use `documents/applications/<company>_<role>/`; if multiple postings share the pair, verify the source URL and ask if ambiguous. Never use a sibling posting’s archive.
...
2. **Fallbacks** (the application may predate `/outcome`): posting via WebFetch on the tracker row's `source` URL, or ask the user to paste it; CV via the tracker row’s `cv_file` (otherwise `cv/main_<company>_<role>.*`) and cover letter via `cover_letter_file` (otherwise `cover_letters/cover_<company>_<role>.*`), where `<company>_<role>` denotes the selected slug from Step 1: use the tracker marker when present, otherwise derive it by the **Subfolder naming** rule in `documents/README.md` for unambiguous legacy rows. For a China text-only row use `china_text_pack:<relative-pack-path>` to see what was drafted and ask what was actually sent; never fetch its job-board URL or substitute a sibling CV or cover letter. **Never widen those globs to the company alone**: with two roles at one company it would prep you from the sibling role's documents. State plainly which context is missing rather than guessing - and suggest `/outcome <company>` to build the archive for next time.
...
- For likely questions **no existing STAR example covers**, draft a new STAR answer grounded strictly in facts from `documents/profile/01-candidate-profile.md` - profile facts arranged into S/T/A/R, not embellished. Include these drafts in the prep pack; offer to append them to `documents/profile/07-interview-prep.md` only if the user explicitly approves.
...
5. **Write only to the application archive** — with one exception. The prep pack lands in the archive folder derived in Step 1; framework files are not edited, except appending user-approved STAR examples to `07-interview-prep.md` on explicit request.
   ...
   **The exception is `documents/profile/01-candidate-profile.md`.** Interview prep is where new facts surface most often: the user recalls a metric, corrects a scope, or fills in a STAR stub. When that happens, write the fact into the local profile, as well as putting it in the prep pack. A fact recorded only in prep material reads as unsupported to a later drafting session and gets stripped from CVs as a fabrication. Prep files are not a substitute for the profile.
