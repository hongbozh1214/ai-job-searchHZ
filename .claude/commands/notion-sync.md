...
1. Check that authenticated Notion tools are available in this session. Tool names
   vary by runtime, so determine this from the session's own tool list **only**;
   never run a provider-specific CLI command as a probe. If the tools are not
   available, stop with one runtime-neutral instruction:
   > Notion is not connected. Connect Notion through your agent runtime's app or MCP settings, start a new session if required, complete OAuth there, and re-run `/notion-sync`.
2. Verify the connection with one cheap call (for example, a workspace search).
   On an authentication error, tell the user to reconnect Notion through the same
   runtime settings and stop. Never retry in a loop.
3. The Notion connection is interactively authenticated, so "connected but not
   authenticable right now" (expired OAuth or a headless/CI context where login
   cannot run) gets the same graceful exit as "not configured": state the reason
   in one line and stop. If only an authentication handshake is available and no
   data tools are exposed, **never initiate OAuth from this workflow and never ask
   whether to authenticate now**. Authentication is the user's action outside the
   workflow.
...
2. Select `seen_jobs.json` entries with status `ranked` whose `rank_score` meets the threshold from Step 0, **only when `rank_eligible` is not `false` and none of `location_verdict`, `language_gate` or the recorded `market_gates` is `FAIL`**. Apply the veto even to older entries without `rank_eligible`, and even when `--all` lifts the score threshold. Tracker rows still join regardless of ranking eligibility: an actual drafted or submitted application remains visible, but do not present its vetoed score as a new job recommendation.
3. Every tracker row joins the sync set (an applied-to job always syncs, ranked or not). Match its `source` URL against the ranked entry URL first, then match company + role only when a URL is absent and the pair is unambiguous. Two rows with the same company/role but different known URLs are two postings and must never be merged. Tracker-only rows use `posting_key:<slug>` or `china_posting_key:<slug>` from notes as their Key when present; otherwise use `<company>_<role>` lowercased with underscores, adding a stable URL suffix if another posting already uses that Key.
...
3. **Links** - the posting URL; select the archive slug from this row’s `posting_key:<slug>` or `china_posting_key:<slug>` marker, or derive `<company>_<role>` by the **Subfolder naming** rule in `documents/README.md` only for unambiguous legacy rows; if that archive exists locally, name its path (plain text - the destination cannot link into the filesystem).
