...
3. Every tracker row joins the sync set (an applied-to job always syncs, ranked or not). Match its `source` URL against the ranked entry URL first, then match company + role only when a URL is absent and the pair is unambiguous. Two rows with the same company/role but different known URLs are two postings and must never be merged. Tracker-only rows use `china_posting_key:<slug>` from notes as their Key when present; otherwise use `<company>_<role>` lowercased with underscores, adding a stable URL suffix if another posting already uses that Key.
