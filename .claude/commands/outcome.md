...
4. Derive the archive folder name from this exact tracker row’s `posting_key:<slug>` or `china_posting_key:<slug>` marker in notes first. Otherwise derive `documents/applications/<company>_<role>/` by the **Subfolder naming** rule in `documents/README.md` for legacy rows. If multiple rows share the company and role, verify the source URL against the archived posting where available; when ambiguous, ask rather than selecting a sibling archive. Use this folder everywhere, including follow-up and stale branches. Check whether the folder and an `outcome.md` already exist - if so, you are updating, not creating.
